...
export interface RateLimitPromisePoolItem<T = any, R = any> {
...
export interface RateLimitPromisePoolCheckpoint<T = any> {
...
export class RateLimitPromisePool {
  ...
  private allCompleteCallback: undefined | (() => void) = undefined;
  ...
  public addCheckpoint(data: any, callback: (data: any) => any) {
    this.checkpointList.push({
      ...
      data,
      position:
        this.executionPool.length +
        this.completedPool.length +
        this.pool.length,
    });
  }
      ...
      if (this.pool.length > 0) {
        if (this.executionPool.length < this.concurrentLimit) {
          ...
          if (now - this.lastCallTimestamp > this.minWaitMs) {
            this.executionPool.unshift(
              this.pool.pop() || ({} as RateLimitPromisePoolItem)
            );
  ...
  /**
   * @description Resolves when either all promises in the pool complete or execution is stopped
   */
  public async allComplete(): Promise<void> {
    return new Promise((res) => {
      this.allCompleteCallback = () => {
        res();
        this.allCompleteCallback = undefined;
      };
    });
  }
  ...
  private checkCheckpoints() {
    this.checkpointList.forEach((checkpoint, index) => {
      if (this.promises.length >= checkpoint.position) {
        Promise.all(this.promises.slice(0, checkpoint.position))
          .then(() => {
